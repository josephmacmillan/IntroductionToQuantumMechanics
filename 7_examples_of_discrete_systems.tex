...
With the exception of Chapter 1, we haven't dealt deeply with any realistic physical systems, but now that all the rules are on the table we can turn to applying them to a few interesting systems.  We'll stick with \emph{two-state systems} for mathematical simplicity, but there are interesting three-state systems (or higher) and all the same rules apply there, too.
...
\begin{figure}
\centering\includegraphics[width=0.5\linewidth]{Figures/Chapter 7/fig_e_in_B_field.pdf}
...
U = -\vec{\mu} \cdot \vec{B}.
\end{equation}
Since our electron is at rest and has no kinetic energy, this is also the full Hamiltonian of our system,
\begin{equation}
...
\begin{figure}
\centering\includegraphics[width=0.5\linewidth]{Figures/Chapter 7/fig_precession_of_spin.pdf}
...
\end{equation}
which  in a way might represent the average direction the spin vector points.  With the time behaviour in the $x$ and $y$ components of the vector, it looks like it precesses  around the $z$ axis, as shown in Figure \ref{fig_precession_of_spin}.  The precession frequency is 
\[
\omega_0 = \frac{eB_0}{m},
\]
...
\ket{\nu} = a \ket{e} + b \ket{\mu}.
\end{equation}
This is a two-state system, something we've already studied in detail, so the physics of neutrinos is pretty much the same as spin-1/2 systems.  To see how neutrinos oscillate, though, we'll have to calculate how this state evolves in time, meaning we'll need the Hamiltonian of the system.
...
\end{pmatrix}.
\]
This matrix is diagonal, so the possible energies are easy to read off: they're both the same, $E = pc$ as we'd expect for a massless relativistic particle.  The eigenstates of the matrix are just the flavour basis states, $\ket{e}$ and $\ket{\mu}$, so the implication is that both neutrinos have the same energy.  More importantly, though, the flavour states -- which are also the energy eigenstates in this massless neutrino scenario -- are stationary states, so neutrino oscillations simply wouldn't happen.  Once an electron neutrino is created during a reaction, it will stay an electron neutrino forever.  

Actually, it's possible to have massive neutrinos and still have the flavour states be stationary, as long as the masses are the same.  In this case the matrix remains diagonal and we reach the same conclusions; evidently it's the \emph{different} masses of the two neutrinos that lead to oscillations.
