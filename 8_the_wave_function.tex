...
\]
Take careful note of the notation; each element in the column vector is a matrix element, and you can clearly see the basis $\ket{+},\ket{-}$ used.  In general, we would have a system that has $n$ elements, where $n$ could be two or three (say, for spin-1) or more -- all the way up to infinity:
\[
...
Now, though, it's time for something a little different (don't worry, only a little).  We want to work in the \emph{position representation},\footnote{Sometimes I'll also call this the position basis, and sometimes position space; all three terms seem to be fairly common.} in which case we might write our state as the column vector
\begin{equation}
...
Like all quantum states we've discussed so far, states written in the position representation must be normalized as well; here, that means
\begin{equation}
...
\end{equation}
being the probability of measuring the state $\ket{\psi}$ to be in the state $\ket{\phi}$.
...
\begin{figure}
\centering\includegraphics[width=0.8\linewidth]{Figures/Chapter 8/fig_example_wave_function.pdf}
...
\begin{figure}
\centering\includegraphics[width=0.8\linewidth]{Figures/Chapter 8/fig_collapsed_wave_function.pdf}
...
Our goal for almost the remainder of the book will be to solve equation (\ref{eq_eee_1d}) -- or its three dimensional version -- for a variety of different potential energies $V(x)$.  Obviously the solutions will depend pretty critically on the exact form of $V(x)$, so it's worthwhile to remind you of how we deal with potential energy \emph{classically} first.

Take a look at Figure \ref{fig_potential_energy_example}, which shows a generic potential energy.  If a particle has a total energy $E$ -- which will always be constant -- as shown in red, then classically the particle is \emph{bound} between the turning points at $x_1$ and $x_2$.  The region outside the turning points is often called the \emph{classical forbidden region}, and although in classical physics the particle will never be found outside the well, in quantum mechanics it is possible to \emph{tunnel} through the barrier and escape, as we'll see later.  

Classically, we distinguish between two types of motion, depending on the value of the energy.  If $E<V_\text{max}$, as shown, then the motion is bound.  However, if $E>V_\text{max}$, we call the motion \emph{unbound} -- although its speed might change as it passes through the potential, it will continue on to infinity and not be trapped.

In quantum mechanics there's an even greater distinction between bound and unbound states.  We'll start our exploration of wave functions by looking at systems that only permit bound states, since they're a little simpler, and then turn to systems that have just unbound states or both bound and unbound together.


\begin{figure}
\centering\includegraphics[width=0.8\linewidth]{Figures/Chapter 8/fig_potential_energy_example.pdf}
\caption{A generic example of a potential energy.  For a particle of total energy $E$, the turning points are at $x_1$ and $x_2$.}
\label{fig_potential_energy_example}
\end{figure}
...
\begin{problem}[Dealing with wave functions]
\label{prob_dealing_with_wave_functions}
...
\item Normalize the wave function (that is, find the value of $A$ such that $\psi(x)$ is normalized).

\item Sketch $\psi(x)$ and $|\psi(x)|^2$.

\item Calculate $\avg{x}$ and $\avg{p}$.

\item Calculate $\avg{x^2}$ and $\sigma_x$.
...
\begin{problem}[Commutator of position and momentum]
\label{prob_commutator}

Show that the commutator of $\hat{x}$ and $\hat{p}$ in position space is 
\[
[\hat{x}, \hat{p}] = i\hbar.
\]
(\emph{Hint:} they're operators, so it's useful to have something to operate on; use a ``test function'' $f(x)$.)

\end{problem}

\begin{problem}[Heisenberg's uncertainty principle]
\label{prob_uncertainty}

Show that the position-momentum uncertainty principle is
\[
